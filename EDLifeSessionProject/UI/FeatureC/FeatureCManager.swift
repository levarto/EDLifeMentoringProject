...
class FeatureCManager: FeatureManager {
    private let store = Store<FeatureCState>.create()
    private let moduleFactory: FeatureCModuleFactory
    ...
    init(moduleFactory: FeatureCModuleFactory) {
    ...
    func canHandle(state: FeatureState) -> Bool {
        return state is FeatureCState
    }

    func handle(state: FeatureState) {
        guard state is FeatureCState else {
